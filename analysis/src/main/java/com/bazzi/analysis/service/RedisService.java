...
public interface RedisService {
    ...
    /**
     * 请求是否被限制，限制返回true，否则false
     * <br/>
     * PS:key中用{}将要hash的字段包含出来，否则会出现CROSSSLOT Keys in request don't hash to the same slot异常
     *
     * @param likeVal         keys对应的值
     * @param count           允许的次数
     * @param countKey        统计次数用的key
     * @param countVal        统计次数用的value
     * @param countExpireTime 统计次数对应key的超时时间 单位:ms
     * @param limitKey        限制key
     * @param limitVal        限制value
     * @param limitExpireTime 限制key对应的超时时间 单位:ms
     * @return 限制:true，不限制：false
     */
    boolean isLimit(String likeVal, long count, String countKey, String countVal, long countExpireTime,
                    String limitKey, String limitVal, long limitExpireTime);

    /**
     * 如果key对应的队列第一个元素和headVal一致，则移除索引为0到num-1的元素，留下num到-1的元素
     *
     * @param key     队列名
     * @param headVal 头元素值
     * @param num     要移除的元素个数
     * @return 删除成功true，反之false
     */
    boolean lTrimNumByHead(String key, String headVal, long num);

    /**
     * 是否需要合并，需要合并返回true，反之false <br/>
     * 逻辑： <br/>
     * 1.先检查mergeKey是否存在，存在直接返回true <br/>
     * 2.将当前命中放入happenKey对应的队列尾部，检查队列长度是否超过calNum <br/>
     * 3.超过calNum，则从队列尾部开始取-calNum值，与当前时间比较是否比calTime大 <br/>
     * 4.如果超过calNum，且时间差小于calTime，则设置mergeKey，并将mergeKey的超时时间为sendInterval，并返回true <br/>
     *
     * 注： <br/>
     * happenKey、mergeKey中用{}将要hash的字段包含出来，否则会出现CROSSSLOT Keys in request don't hash to the same slot异常
     *
     * @param happenKey    命中的队列key
     * @param mergeKey     合并标识key
     * @param calNum       次数
     * @param calTime      时间间隔，秒
     * @param sendInterval 延迟时间，秒
     * @return 需要合并返回true，反之false
     */
    boolean isLogNeedMerge(String happenKey, String mergeKey, int calNum, int calTime, int sendInterval);
}
